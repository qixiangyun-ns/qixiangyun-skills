...
如果没有安装 `login-api-skill`，或没有共享登录态，申报脚本会直接报错并提示先安装/先登录。
...
6. 运行 `scripts/declaration_workflow.py run ...` 前，必须先由 `login-api-skill` 产生共享登录态
...
```bash
python3 scripts/declaration_workflow.py scaffold-config --output /tmp/declaration-config.json
```
